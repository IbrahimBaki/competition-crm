  ...
  return (
    <Card className="p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-lg font-semibold">Knowledge categories</h2><p className="mt-1 text-sm text-slate-500">Organize internal and public guidance.</p></div>
        <Button onClick={() => setEditing(null)}>New category</Button>
      </div>
      <AsyncBoundary query={query} loading={<div className="mt-4"><TableSkeleton columns={4} rows={5}/></div>}>
        {() => <div className="table-scroll mt-4"><table><thead><tr><th>Category</th><th>Code</th><th>Position</th><th>Actions</th></tr></thead><tbody>{rows.map((row) => <tr key={String(row.id)}><td style={{ paddingInlineStart: `${Math.max(0, Number(row.depth) - 1) * 1.25 + 0.75}rem` }}>{String(categoryName(row)?.en ?? categoryName(row)?.ar ?? '')}</td><td>{String(row.code ?? '')}</td><td>{String(row.position ?? 0)}</td><td><Button variant="secondary" onClick={() => setEditing(row)}>Edit</Button></td></tr>)}</tbody></table></div>}
      </AsyncBoundary>
      <ResourceFormDialog
