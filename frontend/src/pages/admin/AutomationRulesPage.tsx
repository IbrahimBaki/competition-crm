...
export function AutomationRulesPage() {
  const { t } = useTranslation(); const { can } = usePermissions(); const [section,setSection]=useState<'rules'|'executions'>('rules'); const [editing,setEditing]=useState<Record<string,unknown>|null|undefined>(undefined); const manage=can(PERMISSIONS.AUTOMATION_RULES_MANAGE);
  return <div><div className="mb-5 flex gap-2" role="tablist" aria-label={t('admin.operations.automation_rules')}><Button role="tab" aria-selected={section==='rules'} variant={section==='rules'?'primary':'secondary'} onClick={()=>setSection('rules')}>{t('admin.operations.automation_rules')}</Button>{can(PERMISSIONS.AUTOMATION_EXECUTIONS_VIEW)&&<Button role="tab" aria-selected={section==='executions'} variant={section==='executions'?'primary':'secondary'} onClick={()=>setSection('executions')}>{t('admin.operations.automation_executions')}</Button>}</div>{section==='rules'?<><CollectionPage title={t('admin.operations.automation_rules')} description={t('admin.operations.automation_rules_description')} endpoint="/automation/rules" queryKey={['automation-rules']} columns={[{key:'name',label:'Rule'},{key:'trigger',label:'Trigger'},{key:'priority',label:'Priority'},{key:'is_active',label:'State'}]} actions={manage?<Button onClick={()=>setEditing(null)}>New rule</Button>:undefined} rowActions={manage?(row)=><Button variant="secondary" onClick={()=>setEditing(row)}>Edit</Button>:undefined} deleteEndpoint={manage?(row)=>`/automation/rules/${String(row.uuid)}`:undefined}/><ResourceFormDialog open={editing!==undefined} title={editing?'Edit automation rule':'New automation rule'} endpoint={editing?`/automation/rules/${String(editing.uuid)}`:'/automation/rules'} method={editing?'PATCH':'POST'} queryKey={['automation-rules']} fields={fields} initial={editing?initial(editing):{priority:100,conditions:[],actions:[],is_active:true,stop_on_match:false}} transform={transform} onClose={()=>setEditing(undefined)}/></>:<CollectionPage title={t('admin.operations.automation_executions')} description={t('admin.operations.automation_executions_description')} endpoint="/automation/executions" queryKey={['automation-executions']} columns={[{key:'executed_at',label:'Executed'},{key:'trigger',label:'Trigger'},{key:'outcome',label:'Outcome'},{key:'ticket_id',label:'Ticket'},{key:'reason',label:'Reason'}]}/>}</div>;
}
