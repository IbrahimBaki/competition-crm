...
import { useTeamListQuery } from '@/features/admin/organisation/useTeamListQuery';
import { TeamListTable } from '@/features/admin/organisation/TeamListTable';

export function TeamsPage() {
  const { t } = useTranslation();
  const { setState, query } = useTeamListQuery();

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">{t('admin.organisation.team.title')}</h1>
      ...
      <div className="rounded border border-gray-200 bg-white">
        <AsyncBoundary
          ...
          empty={<EmptyState title={t('admin.organisation.team.empty_title')} />}
        >
          {(page) => (
            <>
              <TeamListTable rows={page.items} />
